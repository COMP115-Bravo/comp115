...
Your answer is: 
None
Note: the answer is None, instead of [2, 6],  since there is 
no return statement in the function practice_function(nums).
"""
...
Your answer is: 
Yes, because both of these functions return 
the index of the first number whose value equals target, 
and return -1 if there is no elements in nums equal to target.
...
functionality? Why? (5 marks)
Your answer is: 
No. The function practice_for_index_2 returns
the index of the first number whose value equals target, 
because of the key word "break".
The function practice_for_index_3 returns 
the index of the last number whose value equals target.
...
# Write your unit tests below
word_example = "arrrrhhh"
assert good_neighbors(word_example) == 5
